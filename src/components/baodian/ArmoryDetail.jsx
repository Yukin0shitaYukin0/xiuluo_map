import { C, FONT_BODY, FONT_TITLE, TRANS, badge, metalBorder } from './codexStyles';
...
const S = {
  ...
  relRow: { display: 'flex', marginBottom: '4px', fontSize: '12px' },
  relKey: { color: C.textMuted, minWidth: '60px', flexShrink: 0 },
  relVal: { color: C.textSecondary },
};

export default function ArmoryDetail({ weapon, onBack }) {
  if (!weapon) return null;
  const rels = weapon.relations || {};
  ...
  return (
    <div style={S.wrapper}>
      ...
      <h2 style={S.name}>{weapon.name}</h2>
      <div style={S.meta}>
        <span style={badge(C.bronzeLight)}>{weapon.category}</span>
        {weapon.subcategory && <span style={badge(C.goldDim)}>{weapon.subcategory}</span>}
        {(weapon.tags || []).map((t, i) => <span key={i} style={badge(C.textSecondary)}>{t}</span>)}
      </div>
      <div style={S.desc}>{weapon.description || '暂无描述，待修行者补充。'}</div>
